...
APP_SCHEMA_VERSION = "v17_headcount_based_forecast_vp_final"
...
def init_state():
    if st.session_state.get("schema_version") != APP_SCHEMA_VERSION:
        ...
        st.session_state.productive_hours = 7
        st.session_state.working_days = 20
        st.session_state.target_utilization = 90
            ...
            if product in PRODUCTS:
                for year in FORECAST_YEARS:
                    growth_parameters[year][region][product] = {
                        "BAU": safe_int(row[f"{year} BAU"]),
                        "DC": safe_int(row[f"{year} DC"]),
                    }
...
def attrition_to_df(attrition_parameters):
    ...
    for product in PRODUCTS:
        ...
        for year in FORECAST_YEARS:
            row[f"{year} Attr"] = safe_int(
                attrition_parameters[year].get(product, 8)
            )
...
def attrition_df_to_dict(attrition_df):
    ...
    for _, row in attrition_df.iterrows():
        ...
        if product in PRODUCTS:
            for year in FORECAST_YEARS:
                attrition_parameters[year][product] = safe_int(
                    row[f"{year} Attr"]
                )
...
def productivity_to_df():
    return pd.DataFrame(
        [
            {
                "Hrs/Day": safe_int(st.session_state.productive_hours, 7),
                "Days/M": safe_int(st.session_state.working_days, 20),
                "Util %": safe_int(st.session_state.target_utilization, 90),
            }
        ]
    )
...
def productivity_df_to_values(productivity_df):
    ...
    productive_hours = safe_int(row["Hrs/Day"], 7)
    working_days = safe_int(row["Days/M"], 20)
    target_utilization = safe_int(row["Util %"], 90)
...
def get_growth_value(growth_parameters, year, region, product, kind):
    try:
        return safe_int(growth_parameters[int(year)][region][product][kind])
    except Exception:
        return 0
...
def get_attrition_value(attrition_parameters, year, product):
    try:
        return safe_int(attrition_parameters[int(year)][product])
    except Exception:
        return 0
...
def calculate_workforce_headcount(df, growth_parameters, attrition_parameters):
    ...
    for _, row in df.iterrows():
        ...
        baseline_engineers = safe_int(row["Current_SE"])
        ...
        for forecast_year in FORECAST_YEARS:
            ...
            attrition_pct = get_attrition_value(
            ...
            opening_engineers = safe_int(baseline_engineers)

            available_engineers = safe_int(
                opening_engineers * (1 - attrition_pct / 100.0)
            )

            bau_required_engineers = safe_int(
                opening_engineers * (1 + bau_growth_pct / 100.0)
            )

            dc_incremental_engineers = safe_int(
                opening_engineers * (dc_growth_pct / 100.0)
            )

            combined_required_engineers = (
                bau_required_engineers
                + dc_incremental_engineers
            )

            additional_required = max(
                combined_required_engineers - available_engineers,
                0,
            )
...
def build_bu_requirement_comparison(df, result):
    ...
    comparison["Gap / Surplus"] = (
        comparison["Forecast Required SE"]
    ...
    for column in [
        ...
        comparison[column] = comparison[column].round(0).astype(int)
...
def build_actionable_insights(result_all_years):
    ...
    for column in [
        ...
        yearly[column] = yearly[column].round(0).astype(int)

    data = {
        int(row["Forecast Year"]): row
    ...
    if 2027 in data and 2028 in data:
        req_delta_2028 = int(
            data[2028]["Combined Required Engineers"]
            - data[2027]["Combined Required Engineers"]
        )

        hire_delta_2028 = int(
            data[2028]["Combined Additional Required"]
            - data[2027]["Combined Additional Required"]
        )

        direction_2028 = (
            "increase"
            if hire_delta_2028 > 0
            else "reduction"
            if hire_delta_2028 < 0
            else "no change"
        )
        ...
        lines.append(
            f"2028 action: plan for {abs(hire_delta_2028)} SE {direction_2028} "
            f"in hiring demand versus 2027; forecast requirement changes by "
            f"{req_delta_2028:+d} SE."
        )

    if 2028 in data and 2029 in data:
        req_delta_2029 = int(
            data[2029]["Combined Required Engineers"]
            - data[2028]["Combined Required Engineers"]
        )

        hire_delta_2029 = int(
            data[2029]["Combined Additional Required"]
            - data[2028]["Combined Additional Required"]
        )

        direction_2029 = (
            "increase"
            if hire_delta_2029 > 0
            else "reduction"
            if hire_delta_2029 < 0
            else "no change"
        )
        ...
        lines.append(
            f"2029 action: plan for {abs(hire_delta_2029)} SE {direction_2029} "
            f"in hiring demand versus 2028; forecast requirement changes by "
            f"{req_delta_2029:+d} SE."
        )

    year_region = (
        result_all_years
        .groupby(["Forecast Year", "Region"], as_index=False)["Combined Additional Required"]
        .sum()
    )

    if not year_region.empty:
        top_region = year_region.sort_values(
            "Combined Additional Required",
            ascending=False,
        ).iloc[0]

        lines.append(
            f"Deployment focus: highest hiring load is "
            f"{int(top_region['Combined Additional Required'])} SE in "
            f"{top_region['Region']} during {int(top_region['Forecast Year'])}; "
            f"prepare onboarding and deployment capacity for that region first."
        )

    year_product = (
        result_all_years
        .groupby(["Forecast Year", "Product"], as_index=False)["Combined Additional Required"]
        .sum()
    )

    if not year_product.empty:
        top_product = year_product.sort_values(
            "Combined Additional Required",
            ascending=False,
        ).iloc[0]

        lines.append(
            f"BU focus: highest hiring load is "
            f"{int(top_product['Combined Additional Required'])} SE for "
            f"{top_product['Product']} during {int(top_product['Forecast Year'])}; "
            f"prioritize sourcing and training for that BU."
        )

    lines.append(
        "Governance action: review year-wise growth and attrition assumptions monthly, "
        "because 2028 and 2029 depend on the prior year final engineer base."
    )
...
st.sidebar.caption(
    "Headcount-based forecast. Attrition and BAU/DC growth are year-wise. "
...
with st.sidebar.form("planning_assumptions_form"):
    ...
    edited_attrition_df = st.data_editor(
        ...
        column_config={
            ...
            "2027 Attr": st.column_config.NumberColumn(
                "2027 Attr %",
                min_value=0,
                max_value=30,
                step=1,
            ...
            "2028 Attr": st.column_config.NumberColumn(
                "2028 Attr %",
                min_value=0,
                max_value=30,
                step=1,
            ...
            "2029 Attr": st.column_config.NumberColumn(
                "2029 Attr %",
                min_value=0,
                max_value=30,
                step=1,
...
st.info(
    "Upload workforce_input.csv, update year-wise assumptions, click Apply Assumptions, "
...
year_summary = build_year_wise_snapshot(
    df,
    result_all_years,
)

current_base_se = int(round(df["Current_SE"].sum()))

additional_hiring_2027 = int(
    year_summary.loc[
        year_summary["Forecast Year"].astype(int) == 2027,
        "Additional Required SE",
    ].sum()
)

additional_hiring_2028 = int(
    year_summary.loc[
        year_summary["Forecast Year"].astype(int) == 2028,
        "Additional Required SE",
    ].sum()
)

additional_hiring_2029 = int(
    year_summary.loc[
        year_summary["Forecast Year"].astype(int) == 2029,
        "Additional Required SE",
    ].sum()
)
...
total_hiring_2027_2029 = (
    additional_hiring_2027
    + additional_hiring_2028
    + additional_hiring_2029
)

kpi1, kpi2, kpi3, kpi4, kpi5, kpi6 = st.columns(6)

kpi1.metric("Current Base SE", current_base_se)
kpi2.metric("Hiring 2027", additional_hiring_2027)
kpi3.metric("Hiring 2028", additional_hiring_2028)
kpi4.metric("Hiring 2029", additional_hiring_2029)
kpi5.metric("Final SE 2029", final_se_2029)
kpi6.metric("Total Hiring 2027-2029", total_hiring_2027_2029)
...
st.caption(
    "2027 baseline is uploaded Current_SE. "
    "2028 baseline is 2027 Final SE. "
    "2029 baseline is 2028 Final SE. "
    "All SE values are shown as integers."
)
...
with chart_col1:
    product_required = (
        summary_result
        .groupby("Product")["Combined Required Engineers"]
        .sum()
        .reset_index()
    )
...
with chart_col2:
    region_required = (
        summary_result
        .groupby("Region")["Combined Required Engineers"]
        .sum()
        .reset_index()
    )
...
with chart_col3:
    product_hiring = (
        summary_result
        .groupby("Product")["Combined Additional Required"]
        .sum()
        .reset_index()
    )
...
with chart_col4:
    region_hiring = (
        summary_result
        .groupby("Region")["Combined Additional Required"]
        .sum()
        .reset_index()
    )
...
with tab0:
    ...
    executive_summary_table = year_summary[
        [
            "Forecast Year",
            "Baseline SE",
            "After Attrition SE",
    ...
    addl_by_year_exec = (
        result_all_years
        .groupby("Forecast Year")["Combined Additional Required"]
        .sum()
        .to_dict()
    )
    ...
    final_2029_exec = final_se_2029

    forecast_required_2029_exec = int(
        result_all_years.loc[
            result_all_years["Forecast Year"].astype(int) == 2029,
            "Combined Required Engineers",
        ].sum()
    )
    ...
    st.markdown(
        ...
        <div class="leadership-box">
            ...
            <ul>
                <li>
                    Current installed base is
                    <span class="highlight">{current_base_se} SE</span>.
                    Projected final workforce by 2029 is
                    <span class="warning">{final_2029_exec} SE</span>.
                </li>
                <li>
                    Forecast requirement by 2029 is
                    <span class="warning">{forecast_required_2029_exec} SE</span>.
                    Total hiring requirement across 2027 to 2029 is
                    <span class="warning">{total_hiring_exec} SE</span>.
                </li>
                <li>
                    Year-wise hiring ask is
                    <span class="highlight">{hiring_2027_exec} SE in 2027</span>,
                    <span class="highlight">{hiring_2028_exec} SE in 2028</span>,
                    and
                    <span class="highlight">{hiring_2029_exec} SE in 2029</span>.
                </li>
                <li>
                    Input filter:
                    <span class="highlight">Year {selected_input_year_text}</span>,
                    Region
                    <span class="highlight">{selected_region_text}</span>.
                    Forecast years selected:
                    <span class="highlight">{selected_forecast_year_text}</span>.
                </li>
                <li>
                    Planning logic:
                    2027 baseline uses uploaded Current_SE,
                    2028 baseline uses 2027 final engineers,
                    and 2029 baseline uses 2028 final engineers.
                </li>
                {''.join([f'<li>{line}</li>' for line in exec_action_lines])}
                <li>
                    <span class="warning">Decision required:</span>
                    approve year-wise hiring phasing for 2027, 2028, and 2029.
                </li>
                <li>
                    <span class="warning">Recruitment action:</span>
                    confirm whether hiring should be front-loaded in the
                    highest-demand BU and region.
                </li>
                <li>
                    <span class="warning">Execution focus:</span>
                    align sourcing, onboarding, training, deployment readiness,
                    and regional capacity before the hiring peak year.
                </li>
                <li>
                    <span class="warning">Governance cadence:</span>
                    review BAU growth, DC growth, and attrition assumptions monthly,
                    because each forecast year compounds from the previous year final SE base.
                </li>
            </ul>
        </div>
        """,
...
with tab4:
    ...
    st.dataframe(
        add_total_row_and_column(dc_table),
        use_container_width=True,
    )
    ...
    st.dataframe(
        add_total_row_and_column(combined_table),
        use_container_width=True,
    )
...
with tab5:
    ...
    yearly_summary = build_year_wise_snapshot(
        df,
        result_all_years,
    )
    ...
    st.caption(
        ...
        "All headcount values are shown as integers."
    )
